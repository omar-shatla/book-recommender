...
2. **Embedding & semantic search**: Book descriptions are embedded locally using [`BAAI/bge-small-en-v1.5`](https://huggingface.co/BAAI/bge-small-en-v1.5) via `sentence-transformers` — no external API calls, no rate limits, runs entirely on-device. Embeddings are stored and queried using [ChromaDB](https://www.trychroma.com/) through LangChain.
